...
Server::Server(const char *ip, int port)
{
	...
	event_base_dispatch(base);       //监听集合
	 event_base_free(base);          //释放集合 防止内存泄漏
...
void Server::client_handler(int fd)
{
	...
	//创建bufferevent对象 BEV_OPT_CLOSE_ON_FREE关闭TCP连接后会自动释放bufferevent对象不会内存泄漏
	...
	//给bufferevent设置回调函数  //监听之后做什么
	bufferevent_setcb(bev, read_cb, NULL, event_cb, NULL);
	//使能回调函数
	bufferevent_enable(bev, EV_READ);//可读属性
...
{
	char buf[1024] = {0};
	int size = bufferevent_read(bev, buf, sizeof(buf)); //读出数据
	if (size < 0)
	{
