...
void GroupChat::on_sendButton_clicked()
{
    ...
    ui->lineEdit->clear();
    ui->textEdit->append(userName+":"+text);    //显示发送聊天内容
...
void GroupChat::show_group_text(QJsonObject obj)
{
    if (obj.value("cmd").toString() == "group_chat")
    {
        if (obj.value("group").toString() == groupName)
        {
            ...
            this->activateWindow();
            ui->textEdit->append(obj.value("user").toString()+":"+obj.value("text").toString());//显示接收的聊天内容
            ui->textEdit->setAlignment(Qt::AlignLeft); //靠左显示
            ui->textEdit->append("\n");
        }
    }
}
