...
{
    ui->setupUi(this);
   // ui->actionNew->setIcon(QIcon("C:\\Users\\86156\\Desktop\\1\\CKchat\\liaotian.png"));
    userName = u;
    socket = s;
    connect(socket, &QTcpSocket::readyRead, this, &Chatlist::server_reply);

    QStringList friList = fri.split('|');//获取好友列表
    for (int i = 0; i < friList.size(); i++)
    {
        if (friList.at(i) != "")
        {
            ui->friendList->addItem(friList.at(i));//打印到窗口
        }
    }

    QStringList groList = group.split('|');//获取群列表
    for (int i = 0; i < groList.size(); i++)
    {
        if (groList.at(i) != "")
        {
            ui->groupList->addItem(groList.at(i));//打印到窗口
        }
    }
...
void Chatlist::server_reply()
{
    ...
    QJsonObject obj = QJsonDocument::fromJson(ba).object();//获取json内容
    //根据cmd 判断
    ...
    else if (cmd == "private_chat_reply") //不在线
    {
        client_private_chat_reply(obj.value("result").toString());
    }
    else if (cmd == "private_chat")  //在线
    {
...
void Chatlist::client_chat_reply(QJsonObject &obj)
{
    ...
    emit signal_to_sub_widget(obj);//自定义信号
}
...
void Chatlist::on_addButton_clicked() //添加好友button
{
